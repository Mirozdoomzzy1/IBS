...
## v13.1 username authentication

This build uses username/password in the UI instead of email. The initial administrator is `admin` / `123`. Run `SUPABASE-ONE-CLICK.sql` and deploy `supabase/functions/ibs-user-admin` before first use. Administrators can create and disable users and assign roles from Security Center.
