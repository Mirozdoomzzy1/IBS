...
function renderRequirements(){
  ...
  $("#content").innerHTML=`
    ...
    <div class="table-wrap"><table class="table"><thead><tr><th>ID</th><th>Module</th><th>Title</th><th>Actor</th><th>Priority</th><th>Status</th><th>Actions</th></tr></thead><tbody id="reqRows">
      ${arr.length?arr.map(r=>`<tr data-search="${esc((r.id+" "+r.title+" "+r.actor).toLowerCase())}"><td><strong>${esc(r.id)}</strong></td><td>${esc(moduleById(r.moduleId)?.name)}</td><td>${esc(r.title)}${r.comments?`<span class="comment-badge">💬</span>`:""}</td><td>${esc(r.actor)}</td><td><span class="tag ${r.priority==="High"?"orange":""}">${esc(r.priority)}</span></td><td><span class="tag ${r.status==="Approved"?"green":"blue"}">${esc(r.status)}</span></td><td><div class="actions"><button class="icon-btn" data-action="edit-requirement" data-id="${r.id}">✎</button><button class="icon-btn danger" data-action="delete-requirement" data-id="${r.id}">×</button></div></td></tr>`).join(""):`<tr><td colspan="7"><div class="empty"><strong>No requirements yet</strong>Add the first business requirement for this module.</div></td></tr>`}
    </tbody></table></div>`;
...
function erdTable(e,full=false){
  ...
  return `<div class="erd-table ${full?'full-table':''} ${source?'connect-source':''}" data-entity="${e.id}" style="left:${e.x||20}px;top:${e.y||20}px">
    ...
    <div class="erd-table-foot"><span>${renderCommentSummary(e)}</span><span>${relCount} relation${relCount===1?'':'s'}</span></div></div>`;
}
